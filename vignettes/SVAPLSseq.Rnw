...
In this variant a separate expression matrix is first created corresponding to a set of available control genes 
that do not have any differential effect between the two groups. This matrix is only expected to contain the signatures 
of the hidden factors in the data. It is then regressed on the original data to extract the surrogate variables for the 
underlying latent variation. This variant is called by setting the \texttt{controls} argument of the function to a vector of 
indices for the control genes.
